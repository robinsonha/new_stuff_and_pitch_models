...
#s2020 <- read_csv('statcast_2020.csv') #currently unavailable
...
mlbraw <- bind_rows(s2017, s2018, s2019, s2021, s2022, s2023) %>% distinct() %>% filter(p_throws == 'L' & stand == 'R' & game_type == "R")
...
## view feature importance and record as csv for each model
imp <- mlr::getFeatureImportance(xgmodel)
view(imp$res)
write.csv(imp$res,file=paste0("feature_importance_xgmodel",set,".csv"))
...
saveRDS(xgmodel, file = "c_pitch_LHPvsRHBNEW.rds")
